...
| `api($path, $resource)`       | String, String  | null       | URL that can be used to fetch options asynchronously. The search string is provided in the `search` query parameter. The API must return object containing key-value pairs (`['id' => 'value']`). |
| `asyncResource($resource)`    | String          | null       | Provide a Resource class to fetch the options asynchronously.                                                                                                                                     |
